    ...
    return (
      <S.ProductCard>
        <div>
          <img
            src={el.url}
            onClick={() => redirectToProductPage(el.id)}
            alt={el.name}
          />
        </div>
        <div>
          <h3 className="name" onClick={() => redirectToProductPage(el.id)}>
            {el.name}
          </h3>
        </div>
        <div>
          ...
          <p>ou</p>
          <p>
            <strong>em até</strong> 12x de {formatValue(el.price / 12)}
          </p>
        </div>
        <div>
          <S.Button onClick={() => handleAddToCard(el)}>
          ...
          <S.DivFilter>
            <div>
              <input
                ...
                onChange={(el) => handleFilterProducts(el.target.value)}
              />
            </div>
            {categoriesToFilter.map((el) => (
              <h5 onClick={() => handleFilterProducts(el)}>{el}</h5>
            ))}
          </S.DivFilter>
        </div>
        <S.Body>{gotProducts && gotProducts.map(printingProducts)}</S.Body>
      </S.Div>
    </S.Container>
  );
};
